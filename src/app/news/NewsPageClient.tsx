...
import type { HomeNewsData } from '@/lib/storyblok-home';

type CategoryType = 'All' | 'News' | 'Recent';

interface StoryblokNewsArticle {
	id: string;
	title: string;
	excerpt: string;
	date: string;
...
type DisplayArticle = NewsArticle | StoryblokNewsArticle;

interface NewsPageClientProps {
	newsArticles: NewsArticle[];
	storyblokNews: HomeNewsData[];
}
...
function formatStoryblokNews(storyblokNews: HomeNewsData[]): StoryblokNewsArticle[] {
	return storyblokNews
		...
		.map((article) => ({
			...
			date: article.date,
			category: article.category || 'Recent',
			image: article.image,
			featured: false,
			isRecent: true,
			href: article.externalUrl || '#',
			isExternal: Boolean(article.externalUrl),
		}));
}
...
function ArticleCard({ article }: { article: DisplayArticle }) {
	...
	if ('isExternal' in article && article.isExternal) {
		return (
			<a
				href={article.href}
				target="_blank"
	...
	const href = 'href' in article ? article.href : `/news/${article.slug}`;
...
export default function NewsPageClient({ newsArticles, storyblokNews }: NewsPageClientProps) {
	const [activeCategory, setActiveCategory] = useState<CategoryType>('All');
	const contentRef = useRef<HTMLElement>(null);
	const recentArticles = formatStoryblokNews(storyblokNews);
	...
	const filteredArticles = (() => {
		if (activeCategory === 'Recent') {
			return recentArticles.length > 0
				? recentArticles
				: newsArticles.filter((article) => article.isRecent);
		}

		if (activeCategory === 'News') {
			return newsArticles.filter((article) => article.category === 'News');
		}

		return [...recentArticles, ...newsArticles];
	})();
				...
				<div className="container-custom">
					<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
						{filteredArticles.map((article) => (
							<ArticleCard
								key={'slug' in article ? article.slug : article.id}
								article={article}
							/>
						))}
					</div>
