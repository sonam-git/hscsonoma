          ...
          {latestNewsData && latestNewsData.length > 0 && latestNewsData.some(news => news.title) ? (
            <div className={`grid gap-6 ${
              ...
              {latestNewsData.filter(news => news.title).map((news) => (
                <article key={news.id} className="bg-cream-50 dark:bg-mountain-700 rounded-xl p-6 hover:shadow-lg transition-shadow flex flex-col">
                  ...
                  <span className="inline-block px-2 py-1 bg-burgundy-100 dark:bg-burgundy-900/50 text-burgundy-700 dark:text-burgundy-400 text-xs font-medium rounded mb-2 w-fit">
                    {news.category}
                  </span>
                  <h3 className="text-lg font-semibold text-mountain-900 dark:text-cream-50 mb-2 font-[Georgia,'Times_New_Roman',Times,serif]">
                    {news.title}
                  </h3>
                  <p className="text-mountain-600 dark:text-mountain-300 text-sm line-clamp-2 mb-3 font-[Georgia,'Times_New_Roman',Times,serif] flex-grow">
                    {news.excerpt}
                  </p>
                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-cream-200 dark:border-mountain-600">
                    {news.date && (
                    ...
                    {news.externalUrl ? (
                      <a 
                        ...
                        className="inline-flex items-center gap-1 text-sm font-medium text-burgundy-600 dark:text-burgundy-400 hover:text-burgundy-700 dark:hover:text-burgundy-300 transition-colors"
                      >
                        Read More
