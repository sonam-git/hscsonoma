...
// Check STORYBLOK_REGION env var
const region = (process.env.STORYBLOK_REGION as 'us' | 'eu') || 'us';
console.log(`🌍 Storyblok region: ${region} (env: ${process.env.STORYBLOK_REGION || 'not set'})`);
...
// Content version: Always use 'published' with a Public token
// Note: If you have a Preview token and want draft content in development, 
// change this to: const contentVersion = isProduction ? 'published' : 'draft';
const contentVersion = 'published';
...
console.log(`📋 Storyblok params: version=${sbParams.version}, cv=${sbParams.cv}`);
...
const fallbackHero: HomeHeroData = {
  backgroundImage: '/images/hero/hsc.png',
  backgroundAlt: 'Sherpa Sonoma - Himalayan Sherpa Club',
  mobileImage: '/images/hero/hsc-mobile.png',
